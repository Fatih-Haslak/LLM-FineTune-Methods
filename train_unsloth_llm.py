...
RUN_NAME  = f"unsloth-gemma-9b-law-qlora-{datetime.now().strftime('%Y%m%d_%H%M')}"
OUT_DIR   = f"./outputs/{RUN_NAME}"

# ── Donanım ──────────────────────────────────
LOAD_4BIT = True
DTYPE     = None            # None = auto-detect (fp16 / bf16)
...
# ── Kaydetme Formatları ──────────────────────
SAVE_LORA_ADAPTER  = True     # LoRA adapter (küçük, ~100 MB)
SAVE_MERGED_16BIT  = True     # Base + LoRA birleşik full model (büyük, ~18 GB)
...
def main():
    ...
    log.info(f"Model yükleniyor (Unsloth): {MODEL_ID}")
    ...
    # ── LoRA (Unsloth) ────────────────────────
    log.info("LoRA ekleniyor (Unsloth)...")
    ...
    model = FastLanguageModel.get_peft_model(
        ...
        bias="none",
        use_gradient_checkpointing="unsloth",   # Unsloth optimize GC → %30 daha az VRAM
        random_state=SEED,
    )
    ...
    dataset = dataset.map(format_as_chat, num_proc=1)
    ...
    # ── Eğitim ────────────────────────────────
    log.info("=" * 60)
    log.info("EĞİTİM BAŞLIYOR (Unsloth)")
    ...
    # ── Kaydetme ──────────────────────────────
    # 1) LoRA adapter — küçük dosya (~100 MB)
    #    inference_llm.py ile PeftModel.from_pretrained() ile yüklenir
    if SAVE_LORA_ADAPTER:
        lora_dir = f"{OUT_DIR}/lora_adapter"
        log.info(f"LoRA adapter kaydediliyor → {lora_dir}")
        model.save_pretrained(lora_dir)
        tokenizer.save_pretrained(lora_dir)

    # 2) Merged 16-bit full model — büyük dosya (~18 GB)
    #    AutoModelForCausalLM.from_pretrained() ile doğrudan yüklenir
    #    adapter gerekmez, bağımsız çalışır
    if SAVE_MERGED_16BIT:
        merged_dir = f"{OUT_DIR}/merged_16bit"
        log.info(f"Merged 16-bit model kaydediliyor → {merged_dir}")
    ...
    log.info("=" * 60)
    log.info("✅ Eğitim tamamlandı (Unsloth).")
    if SAVE_LORA_ADAPTER:
        log.info(f"   LoRA adapter  : {OUT_DIR}/lora_adapter")
    if SAVE_MERGED_16BIT:
        log.info(f"   Merged model  : {OUT_DIR}/merged_16bit")
